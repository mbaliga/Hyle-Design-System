 ...
 * deliberately, and the way it fails is deliberate too. Ports the intent of this repo's own
 * never-merged PR #15 onto current state (that PR is stale reference material — its exact
 * numbers are superseded below, its shape is not).
 *
 ...
 * ## Migrating
 *
 * See `crash-recovery/MOVED.md` here, or that repo's own README/MIGRATION.md. In short:
 *
 *     // settings.gradle.kts
 ...
 *
 * The package (`dev.aarso.crashrecovery`) and the API are unchanged for every member this stub
 * mirrors, so no import or call-site edits are needed for them. If this repo's submodule existed
 * *only* for crash-recovery — as it did for BOS_launcher, Clackpad and hnm_playground — remove
 * `hyle-design-system` entirely.
 *
 * Note that 1.5.0 is not simply the old 1.1.0 renamed: it folds forward the `previewIntent` /
 * `samplePreview` work from the never-merged `33b0faa` branch (which Android-IDE-core pinned and
 * calls — those two histories had diverged and neither was a superset), plus the "Continue"
 * relaunch fix and the non-destructive quarantine/salvage reset that landed after this repo's own
 * copy stopped evolving. None of that later API is mirrored below — nothing in this repo's
 * history ever called it, so there is no existing call site for it to fail loudly at.
 */
...
private const val MOVED: String =
    "dev.aarso:crash-recovery has MOVED to mbaliga/Shared-Libraries-asoc. " +
        ...
        "dev.aarso:crash-recovery:1.5.0 from there. The package and this stub's API are " +
        "unchanged, so no code edits are needed for the members it mirrors. If this repo's " +
        "hyle-design-system submodule existed only for crash-recovery, remove it. See " +
        "crash-recovery/MOVED.md."
 ...
 *
 * Signatures mirror the module as it stood at 1.1.0 (plus `previewIntent`, kept for the one real
 * pinned-branch caller that used it) so a consumer's *existing* call sites still bind here and
 * produce the migration error, rather than an "unresolved reference" that would be no more
 * useful than the missing artifact.
 */
